...
# Fix Code - Map Filter to Prompt node (use simple UUID, no crypto)
$fixedFilterCode = @'
...
# Fix Code - Generate Signed URL node (simplified, no crypto)
$fixedSignedUrlCode = @'
// Generate pre-signed URL for R2 (simplified without crypto)
// Since we're using S3 node for upload, we can use public URL or simplified approach
...
// Use R2's public URL if bucket is public, or construct download URL
